...
!  for setup:
   ...
   integer zlen,nftszstart,nfil,jtreadmil,jdepo,intering,nphisol,nxsol,nwall,nbondwal,nw,nb,jx,jp
   integer nmeho,nhoop,nmemb,nm,jh,jm,nftsz,fstart0,length,length0,nzbond,n,jf,jm0
   integer npick,nbox,jb,nftsa,nftsamax,tension,nring,nr
   integer nfilmax,nsunit,jgradient,jnogradient,jxwid
   integer npoly,nmono,nx,nx0,nphi,nphi0,jocc,jdir,jbend,jskip,nskip,nskip1,nskip2,overlap,ncover

   integer,allocatable,dimension(:,:)::bondwal,fill
   integer,allocatable,dimension(:)::fil2a,a2mem,a2fil,fstart,flen,filid,ringid,gtp,mark

   real(kind=8)::rate,runtime
   real(kind=8)::mbrad,mwid,rrad,rwid,pi,delta,invdelta,beta,k_z,l_z,k_a,l_mb_a,ktether,l_a_z
   real(kind=8)::kz_thet,thet0_z,ltether,p_tether,l_mem,k_mem!,l_z12,l_z34,dl_z,l_diag_new
   ...
   real(kind=8)::xorigin,angle,wallwidth,viscos,fgrad,kwid,kgly,lpep,kpep,fturgor,pturgor,l0,dxshift,rcutoff
...
!  for dynamics
   ...
   real(kind=8)::ptread,phyd,padd,tadd,pdepo,k_scale,k_scale_mb,kmemb!_thet,thet_memb
   ...
   real(kind=8)::printtime,oldtime,dtmodz,fmaxmb2,wallcons,dtime,oldwallcons
!   real(kind=8)::fa1max2,fa2max2,fa3max2,fa4max2,fb1max2,fb2max2,fb3max2,fb4max2,facenmax2
   real(kind=8)::fmax,knormal,growstep,kcir,kcircon,rswitch,fswitch,dtremod,rateremod

   real(kind=8),allocatable,dimension(:)::fxmemb,fymemb,fzmemb,fxfil,fyfil,fzfil
   real(kind=8),allocatable,dimension(:,:)::fx_a,fy_a,fz_a
